...
--
-- Máy chủ: 127.0.0.1
-- Thời gian đã tạo: Th9 22, 2024 lúc 04:44 PM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_music`  AS SELECT `bv`.`ma_bviet` AS `ma_bviet`, `bv`.`tieude` AS `tieude`, `bv`.`ten_bhat` AS `ten_bhat`, `tg`.`ten_tgia` AS `ten_tgia`, `tl`.`ten_tloai` AS `ten_tloai`, `bv`.`ngayviet` AS `ngayviet`, `bv`.`hinhanh` AS `hinhanh` FROM ((`baiviet` `bv` join `tacgia` `tg` on(`bv`.`ma_tgia` = `tg`.`ma_tgia`)) join `theloai` `tl` on(`bv`.`ma_tloai` = `tl`.`ma_tloai`)) ;
...
--
ALTER TABLE `theloai`
  MODIFY `ma_tloai` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=53;
